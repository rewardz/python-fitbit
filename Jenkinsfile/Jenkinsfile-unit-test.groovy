...
''' }
  ...
  environment {
    ...
    FINAL_IMAGE = 'python-fitbit'
    PR_NUM = "${env.CHANGE_ID}"
    NAMESPACE = "pr-python-fitbit-${PR_NUM}"
  ...
  stages {
    stage ('init') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: "${env.CHANGE_BRANCH}"]], extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'python-fitbit']], userRemoteConfigs: [[credentialsId: 'github-app-rewardz', url: 'https://github.com/rewardz/python-fitbit.git']]])
          ...
          steps {
              container('backend') {
                  script {
                      try {
                          sh """
                            cd /code && python -m coverage run -m unittest discover -s fitbit_tests && python -m coverage report -m
                          """
                      } catch (Exception e) {
                          echo "Tests failed"
                          throw e
